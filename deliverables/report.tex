...
Converting this continuous score into a binary detection decision requires a threshold; the threshold selection procedure and its limitations are discussed in Section~\ref{sec:method:thresholds}.
...
As a result, any deviation between the code and the algorithmic descriptions in Section~\ref{sec:method:thresholds:spot} reflects choices made in Siffer et~al.'s own implementation, not modifications introduced in this project.
...
Fabre and Challet~\cite{fabre_2025} use this condition directly as their detection rule, labelling as suspicious all large orders ($Q \geq 4{,}500$~USD) for which $G_t > 0$.
...
The values of $Q$, $q$, $\delta_a$, $\varepsilon^+$, and $\varepsilon^-$ follow Fabre and Challet~\cite{fabre_2025}.
...
\textbf{Parameter} & \textbf{Symbol} & \textbf{Value} & \textbf{Source} \\
\midrule
Spoof order size & $Q$ & 4{,}500 & Fabre and Challet~\cite{fabre_2025} \\
...
Spoof order distance & $\delta_b$ & 0.01 & Empirical \\ % TODO: no systematic justification; matches deepest grid point in the paper
Maker fee & $\varepsilon^+$ & 0 & Fabre and Challet~\cite{fabre_2025} \\
Taker fee & $\varepsilon^-$ & 0.05 & Fabre and Challet~\cite{fabre_2025} \\
\bottomrule
\end{tabular}
\end{table}
...
Accordingly, the implementation evaluates the cost-function counterfactual using a single set of PNN-predicted parameters $(\mu_t, \sigma_t, \alpha_t)$: the no-spoof cost sets $Q = 0$ and $\delta_b = 0$, while the with-spoof cost uses the actual parameter values.
This captures the mechanical effect of different fill thresholds but does not capture the distributional shift that the spoof order would induce in the Hawkes features and hence in the predicted $(\mu_t, \sigma_t, \alpha_t)$.
...
The EVT-based (POT, SPOT, DSPOT) and rolling FDR methods described in Section~\ref{sec:method:thresholds} can alternatively be applied to the raw gain series, treating it as an anomaly score and selecting a threshold that controls the tail probability or false discovery rate.
