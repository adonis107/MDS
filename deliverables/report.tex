...
The value $Q = 1{,}000$ was chosen empirically to represent a moderately sized institutional order for TOTF.PA; no reference paper prescribes a specific target size, and a sensitivity analysis over $Q$ is deferred to Chapter~\ref{chap:data}.
...
For each side $s$, a linear regression of price on log cumulative volume across the first $D = 5$ levels gives (Tao et~al.~\cite{tao_2020} calibrate a comparable depth parameter to values between 3 and 5 depending on the stock; Fabre and Challet~\cite{fabre_2025} use $K = 5$ as a representative depth):
The depth $D = 5$ is consistent with the range used by Tao et~al.~\cite{tao_2020} (who calibrate 3 to 5 depending on the stock) and Fabre and Challet~\cite{fabre_2025} (who use $K = 5$); an ablation over $D$ is reported in Chapter~\ref{chap:data}.
\begin{equation}
  \label{eq:book_slope}
...
The half-lives $h_s = 5$ and $h_\ell = 50$ were chosen empirically to capture short-term (a few events) and medium-term (tens of events) order flow memory at the top of the book; no reference paper prescribes specific values, and a sensitivity analysis is deferred to Chapter~\ref{chap:data}.
...
The value $W_{\text{warmup}} = 3{,}000$ was chosen empirically to ensure that all rolling windows (up to $W = 100$) and EWMA features (with half-life up to $h_\ell = 50$) are fully populated before the output is consumed by any model; a sensitivity analysis is reported in Chapter~\ref{chap:data}.
...
The anomaly score is the negated OC-SVM decision function, so that higher values indicate greater anomalousness.

\paragraph{Encoder.}
The input to the encoder is a subsequence $\mathbf{S}_t \in \mathbb{R}^{k \times d}$, where $k$ is the sequence length and $d$ the number of features.
A linear embedding projects each feature vector from $\mathbb{R}^d$ to $\mathbb{R}^{d_{\text{model}}}$.
...
This matrix passes through $N_{\text{dec}}$ Transformer layers (using the same \texttt{TransformerEncoderLayer} architecture as the encoder, without cross-attention or causal masking) and a final linear projection from $\mathbb{R}^{d_{\text{model}}}$ to $\mathbb{R}^{d}$ produces the reconstruction $\hat{\mathbf{S}}_t$.
...
\paragraph{Divergences from Poutr\'e et~al.}
The architecture parameters differ from those of Poutr\'e et~al.~\cite{poutre_2024}, who use $h = 8$ attention heads, a feedforward dimension $d_{\text{ff}} = 512$, and $N = 6$ Transformer layers, following Vaswani et~al.~\cite{vaswani2017attention}.
Our implementation uses $h = 4$, $d_{\text{model}} = 64$ (with the default feedforward dimension $d_{\text{ff}} = 2{,}048$ from PyTorch's \texttt{TransformerEncoderLayer}), and $N = 2$, resulting in a substantially smaller model.
This reduction was necessary because the TOTF.PA dataset is smaller than the five-stock LOBSTER dataset used in the original paper, and the larger architecture overfitted in preliminary experiments.
The representation dimension ($d_{\text{rep}} = 128$) and the sequence length ($k = 25$) match the paper.
...
First, the bottleneck is a lower-dimensional ($d_{\text{rep}} = 128$ vs.\ $d \approx 80$ features $\times$ $k = 25$ time steps $= 2{,}000$ raw dimensions), semantically compressed space where the OC-SVM can learn a tighter boundary.
...
The Nystr\"om kernel approximation is a computational adaptation introduced in this project; it does not appear in Poutr\'e et~al.~\cite{poutre_2024}, who use the standard quadratic-programming solver from scikit-learn.
The adaptation is motivated by scalability: a standard kernel SVM requires computing and storing an $n \times n$ kernel matrix, which is infeasible when $n$ exceeds $10^5$ training subsequences.
...
Model dimension & $d_{\text{model}}$ & 64 & Empirical \\
Attention heads & $h$ & 4 & Empirical \\
Transformer layers & $N$ & 2 & Empirical \\
...
The PNN takes as input the flattened feature vector $\mathbf{x}_t \in \mathbb{R}^{k \cdot d}$, where $k = 25$ is the sequence length and $d$ the number of features.
...
\begin{enumerate}
  ...
  The PNN receives the full feature set, flattened over the sequence window.
  \item \textbf{Batch size.} Fabre and Challet use a batch size of $2^{12} = 4{,}096$.
  Our implementation uses a batch size of 64, constrained by GPU memory when the flattened input dimension is large ($k \cdot d \approx 2{,}000$).
...
\textbf{Parameter} & \textbf{Symbol} & \textbf{Value} & \textbf{Source} \\
\midrule
Input dimension & --- & $k \cdot d$ (flattened) & Fabre and Challet~\cite{fabre_2025} \\
...
\begin{equation}
  \label{eq:prae_gate}
  z_i = \mathrm{clamp}\!\bigl(\mu_i + \varepsilon_i,\; 0,\; 1\bigr), \qquad \varepsilon_i \sim \mathcal{N}(0, \sigma^2),
\end{equation}
where $\sigma$ is a fixed noise scale.
When $\mu_i$ drifts below zero, $z_i$ is clamped to zero with high probability, effectively removing sample $i$ from the reconstruction loss.
When $\mu_i$ is large and positive, $z_i \approx 1$ and the sample contributes fully.

\paragraph{Loss function.}
Following the $\ell_1$ relaxation of Lindenbaum et~al.~\cite{lindenbaum2022prae} (their Equation~5), the training objective is:
\begin{equation}
  \label{eq:prae_loss}
  \mathcal{L}_{\text{PRAE}} = \frac{1}{|\mathcal{B}|}\sum_{i \in \mathcal{B}} z_i \left\|\mathbf{S}_i - \hat{\mathbf{S}}_i\right\|^2 - \lambda \cdot \frac{1}{|\mathcal{B}|}\sum_{i \in \mathcal{B}} \mathbb{E}[z_i],
\end{equation}
where the first term is a Monte Carlo estimate of the gated reconstruction error (using the sampled $z_i$) and the second term is the regulariser that encourages including more samples.
The expected gate value $\mathbb{E}[z_i]$ is computed analytically from the truncated Gaussian distribution of $z_i$ given $\mu_i$ and $\sigma$:
\begin{equation}
  \label{eq:expected_gate}
  \mathbb{E}[z_i] = \frac{\sigma}{\sqrt{2\pi}}\left(e^{-\mu_i^2/(2\sigma^2)} - e^{-(1-\mu_i)^2/(2\sigma^2)}\right) + (\mu_i - 1)\,\Phi\!\left(\frac{1-\mu_i}{\sigma}\right) - \mu_i\, \Phi\!\left(\frac{-\mu_i}{\sigma}\right) + 1,
\end{equation}
where $\Phi$ is the standard normal CDF.
Using the analytical expectation rather than a Monte Carlo estimate provides smooth gradients for $\mu_i$ even when $z_i$ is clamped to 0 or 1, which stabilises training.
...
Per-sample reconstruction errors are clamped to $10^6$ to prevent overflow: when a gate drifts to zero, the backbone receives no gradient for that sample, its reconstruction error can grow unboundedly, and the product $0 \cdot \infty$ produces NaN under IEEE~754 arithmetic.
...
The PRAE paper~\cite{lindenbaum2022prae} does not prescribe a specific $\lambda$ selection procedure; it treats $\lambda$ as a hyperparameter to be set based on a contamination-level estimate.
Our implementation uses a two-stage approach.

On the first training day, an initial $\lambda_{\text{heuristic}}$ is computed from the average energy of the training data:
\begin{equation}
  \label{eq:lambda_heuristic}
  \lambda_{\text{heuristic}} = \overline{\|\mathbf{x}\|^2} \cdot k \cdot d,
\end{equation}
where $\overline{\|\mathbf{x}\|^2}$ is the mean squared magnitude per element over a sample of training batches.
...
The factor that minimises the validation reconstruction loss (without gates) is selected.
...
$\lambda$ selection & --- & Grid search (7 factors $\times$ 15 epochs) & Empirical \\
