...
The architecture follows Poutr\'e et~al.~\cite{poutre_2024}, who set the hyperparameters similarly to Vaswani et~al.~\cite{vaswani2017attention}: $h = 8$ attention heads, a feedforward dimension $d_{\text{ff}} = 4d = 512$, $N = 6$ Transformer encoder/decoder layers, and a representation dimension $d_{\text{rep}} = d_{\text{model}} = d = 128$.
...
\begin{equation}
  \label{eq:prae_loss}
  \mathcal{L}_{s} = \mathbb{E} \left( \sum_i z[i] \|x_i - \hat{x}_i \|_2^2 - \lambda \|z\|_s\right) \quad s \in \{0, 1\},
\end{equation}
...
The expected gate value $\mathbb{E}[z_i]$ is computed analytically from the truncated Gaussian distribution of $z_i$ given $\mu_i$ and $\sigma$ (derived in the proof of Theorem~4.1 by Lindenbaum et~al.~\cite{lindenbaum2022prae}):
\begin{equation}
...
\end{equation}
where $\overline{\|\mathbf{x}\|^2}$ is the mean squared magnitude per element, and scaling by the sequence length $k$ and feature dimension $d$ recovers the mean total energy per sample (i.e.\ $\mathrm{ME}$).
...
Each detection model outputs a scalar anomaly score $s_t \in \mathbb{R}$ at every time step $t$.
A detection is triggered when $s_t$ exceeds a threshold $\tau_t$, producing the binary indicator $\mathbf{1}[s_t > \tau_t]$.
The choice of $\tau_t$ governs the trade-off between false positives and detection power.
A fixed threshold is inadequate because anomaly score distributions are non-stationary: volatility regimes shift within and across sessions, market microstructure changes between instruments, and the score baselines of different models drift over time.
The three methods below address this from different angles: static extreme value fitting (POT), streaming extreme value updating (SPOT, DSPOT), and rolling multiple-testing control (RFDR).
Throughout, $s_t$ denotes the anomaly score at time $t$ and $\tau_t$ denotes the (possibly time-varying) threshold.

\subsection{Peak Over Threshold}\label{sec:method:thresholds:pot}

\paragraph{Theoretical foundation.}
The Pickands-Balkema-de Haan theorem~\cite{balkema1974residual,pickands1975statistical} guarantees that, for a wide class of distributions, the excess of a random variable above a sufficiently high threshold~$u$ converges in distribution to a Generalised Pareto Distribution (GPD).
...
\textbf{Parameter} & \textbf{Symbol} & \textbf{Value} & \textbf{Source} \\
\midrule
Risk level & $q$ & $10^{-4}$ & Empirical (paper uses $10^{-3}$) \\
...
SPOT/DSPOT init size & $n_{\text{init}}$ & $\lfloor n/2 \rfloor$ & Empirical (paper suggests ${\sim}1000$) \\ % TODO: no systematic justification for n/2
DSPOT window depth & $d$ & 50 & Empirical (paper uses 200) \\ % TODO: no justification for d=50; paper uses d=200 on network data
\bottomrule
\end{tabular}
\end{table}
...
Rolling window length & $W$ & 500 & Empirical \\ % TODO: no systematic justification for W=500
