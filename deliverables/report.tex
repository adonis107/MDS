...
Before model input, features are further transformed by an empirical Box-Cox scaler (\texttt{EmpiricalBoxCoxScaler}), which applies three operations per feature: (i)~a positive shift ($x \leftarrow x + |\min(x)| + 1$) to ensure strict positivity, (ii)~a Box-Cox power transform with MLE-estimated $\lambda$, and (iii)~z-score standardisation.
...
\begin{itemize}
  \item \textbf{Transformer + OC-SVM}: an event is flagged as anomalous if the OC-SVM decision function is positive (i.e.\ the sample lies outside the learned support).
