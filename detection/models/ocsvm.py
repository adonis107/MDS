"""
One-Class SVM with NystrÃ¶m RBF kernel approximation - pure PyTorch / CUDA.

Replaces the sklearn ``OneClassSVM`` with a GPU-native pipeline:

1. NystrÃ¶m approximation - sample m landmark points to build a
...
class OCSVM(nn.Module, BaseDetector):
    """NystrÃ¶m-approximated One-Class SVM.
    ...
    n_components : int
        Number of NystrÃ¶m landmarks (approximation rank).
    kernel : str
    ...
    def _nystroem_features(self, X: torch.Tensor) -> torch.Tensor:
        """phi(X) via NystrÃ¶m: K(X, landmarks) @ normalization."""
        K = self._rbf_kernel(X, self._landmarks)
        return K @ self._normalization

    def fit(self, X):
        """Fit the NystrÃ¶m OC-SVM.
    ...
    def decision_function(self, X):
        ...
        Positive â†’ inlier, negative â†’ outlier (sklearn convention).
    ...
    def dissimilarity_score(self, X):
        r"""Continuous dissimilarity score from PoutrÃ© et al. (2024), Â§3.5.
        ...
        Reference: PoutrÃ©, C., ChÃ©telat, D., & Morales, M. (2024).
